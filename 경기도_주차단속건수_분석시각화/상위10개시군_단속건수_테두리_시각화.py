...
def visualize_top10_violations_with_border(df):
    """상위 10개 시군 단속건수 시각화 (검정색 테두리 포함)"""
    ...
    if 'SIGUN_NM' in df.columns and 'COLLECT_CNT' in df.columns:
        ...
        # 상위 10개 시군 추출
        top10 = sigun_summary.nlargest(10, '총단속건수')
        
        # 색상 설정 (수원시: #144193, 나머지: #8B9091)
        colors = []
        for sigun in top10['시군명']:
            if sigun == '수원시':
        ...
        # 막대그래프 생성 (검정색 테두리 추가)
        bars = plt.bar(top10['시군명'], top10['총단속건수'], 
                      color=colors, alpha=0.8, width=0.7, 
                      edgecolor='black', linewidth=1.5)  # 검정색 테두리 추가
        
        # 막대 위에 값 표시
        for i, (bar, value, sigun) in enumerate(zip(bars, top10['총단속건수'], top10['시군명'])):
            if sigun == '수원시':
                plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(top10['총단속건수'])*0.01,
                        f'{value:,}', ha='center', va='bottom', fontweight='bold', fontsize=12, color='#144193')
            else:
                plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(top10['총단속건수'])*0.01,
                        f'{value:,}', ha='center', va='bottom', fontweight='bold', fontsize=10, color='#8B9091')
        ...
        # 파일 저장
        plt.savefig('상위10개시군_단속건수_테두리.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        # CSV 파일로 저장
        top10.to_csv('상위10개시군_단속건수_테두리.csv', index=False, encoding='utf-8-sig')
        
        print("=== 상위 10개 시군 단속건수 (테두리 포함) ===")
        print(top10)
        print(f"\n시각화가 '상위10개시군_단속건수_테두리.png' 파일로 저장되었습니다.")
        print(f"데이터가 '상위10개시군_단속건수_테두리.csv' 파일로 저장되었습니다.")
...
def main():
    """메인 실행 함수"""
    print("상위 10개 시군 단속건수 시각화 (테두리 포함)를 시작합니다...")
    ...
    # 3. 상위 10개 시군 시각화 (테두리 포함)
    print("3. 상위 10개 시군 단속건수 시각화를 진행하는 중...")
    visualize_top10_violations_with_border(df)
